...
class HandposeExtension {
  constructor (runtime) {
      ...
      // .then の後に .catch() を追加してエラーハンドリング
      this.runtime.ioDevices.video.enableVideo().then(this.detectHand).catch(error => {
    ...
    */
    ...
    videoToggle (args) {
      ...
      if (state === 'off') {
        ...
        this.runtime.ioDevices.video.enableVideo().then(this.detectHand).catch(error => {
            console.error('Error enabling video in videoToggle:', error);
        });
        //this.runtime.ioDevices.video.enableVideo().then(this.detectHand);
     ...
     // A scratch command block handle that configures the video preview'
...
module.exports = { HandposeExtension, Scratch3Handpose2ScratchBlocks };
//export Scratch3Handpose2ScratchBlocks;
